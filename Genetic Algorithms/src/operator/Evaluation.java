...
public class Evaluation {
	 ...
	 * @param pruferNumberCode : the array of prufer number code
	 * @param num_vertices : the number of vertices
	 * @return tree after apply make tree prufer number algorithm  to prufer number code
	 */
	public  double[][] pruferNumber(int[] pruferNumberCode, int num_vertices){
	
		double[][] tree = new double[num_vertices][num_vertices];
		for(int i=0;i<num_vertices;i++){
			for(int j=0;j<num_vertices;j++) tree[i][j] = -0.001;
		}
	...
	public  double[][] decodingPruferNumber(int[] pruferNumberCode,int[] startpoint,
			ArrayList<Cluster> clusters, ArrayList<Cluster> maxClusters, int num_vertex, Random rnd){
		...
		for(int i = 0; i < numberOfCluster; i++){
			...
			if( i < numberOfCluster - 2 &&  pruferNumberCode[startpoint[maxNumberOfCluster - 1] + i] < numberOfCluster ){
 			presentationClusterPrufer[i] = pruferNumberCode[startpoint[maxNumberOfCluster - 1] + i];
 			    count1++;
			}
			...
			for( int j = 0; j < numberClusterVertices; j++ ){
				for(int k = 0; k < numberClusterVertices; k++){
					tree[clusters.get(i).getCluster().get(j)][clusters.get(i).getCluster().get(k)]
					= clusterTree[j][k];	
				}
			}
//			pruferNumberCode[startpoint[maxNumberOfCluster - 1] + maxNumberOfCluster - 2 + i];
			int position  = pruferNumberCode[startpoint[maxNumberOfCluster - 1] + maxNumberOfCluster - 2 + i];
//			int position  = rnd.nextInt(numberClusterVertices);  
...
//		Paint  p = new Paint();
//		 p.setPaint(tree, ReadFiles.vertices1, ReadFiles.clusters1, ReadFiles.num_vertex1, 
//				 0, 0, ReadFiles.root1);
//		  windows.addPaint(p);
		return tree;
...
//					 }
//					 else{
//						 presentationClusterPrufer[i] = pruferNumberCode[startpoint[maxNumberOfCluster - 1] + i] % numberOfCluster;
//						 
//					 }
//				}
//				
...
//						pruferNumberCodeCluster[j] = pruferNumberCode[j];
//						}else{
//							pruferNumberCodeCluster[j] = pruferNumberCode[j] % numberClusterVertices;
//						}
//						
//					}else {
//						int temp = pruferNumberCode[startpoint[i - 1] + j] ;
//						if(pruferNumberCode[startpoint[i - 1] + j] < numberClusterVertices){			
//						pruferNumberCodeCluster[j] = pruferNumberCode[startpoint[i - 1] + j];
//						}
//						else{
//							pruferNumberCodeCluster[j] =pruferNumberCode[startpoint[i - 1] + j] % numberClusterVertices;
//						}
//						}
//				}
//			
...
//			return tree;
	...
	/**
	 * In order to build up information  for dividing prufer number code to  prufer number code for each cluster.
	 * @param maxClusters
	 * @return  array of start point: each element is value of start point of prufer number of each cluster
	 */

	public  int[] getStartPoint( ArrayList<Cluster> maxClusters){
		int numberOfMaxCluster = maxClusters.size();
	int[] startpoint = new int[numberOfMaxCluster];// start point to divide the prufer number code to build tree for each cluster.
	 ...
	 */
	public  double[][] decodingPruferNumber(int[] pruferNumberCode,int[] startpoint,
			ArrayList<Cluster> clusters, int num_vertex){
...
//		  windows.addPaint(p);
	...
	public double[] getPopFitness(Population population, int populationLength ){
		...
		for( int i = 0; i < populationLength; i++){
			double[][] tree = decodingPruferNumber(population.getIndividual(i).getGene1(), getStartPoint(ReadFiles.clusters),
