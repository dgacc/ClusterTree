...
public class Crossover {
				...
				for (int k = i + 1; k < numberOfCluster; k++) {
					...
					for (int t = 0; t < numberClusterVertex2; t++) {
						if (parent[clusters.get(i).getCluster().get(j)][clusters.get(k).getCluster().get(t)] > 0) {
							tree[i][k] = parent[clusters.get(i).getCluster().get(j)][clusters.get(k).getCluster()
									.get(t)];
							tree[k][i] = tree[i][k];
						}
					}
	...
	public double[][] clusterCrossover(double[][] father, double[][] mother, int num_vertex,
			ArrayList<Cluster> clusters, Random r) {
			...
			for (int j = 0; j < numberClusterVertex; j++) {
				for (int k = 0; k < numberClusterVertex; k++) {
					Tree[clusters.get(i).getCluster().get(j)][clusters.get(i).getCluster()
							.get(k)] = spanningTreeOfCluster[j][k];
				}
			}
	...
	/**
	 * apply BFS for Crossover operation The first: choose the representation
	 * vertex for each cluster The second: apply BFS to generate the tree for
	 * each cluster The third: apply BFS for big Tree then copy the tree of
	 * cluster and big tree to Tree.
	 * 
	 * @param father
	 * @param mother
	 * @param num_vertex
	 ...
	 *            : tree
	 * @param num_vertex
	 *            : number of vertices of tree
	 * @param maxClusters
	 *            : clusters
	 * @param numberOfCluster1
	 *            : numberOfCluster1 < numberOfCluster2
	 * @param numberOfCluster2
	 *            : numberOfCluster1 < numberOfCluster2
	 * @param minClusterVertices
	 *            : //array of number of vertex in cluster have less than the
	 *            other.
	 * @param maxClusterVertices
	 *            //array of number of vertex in cluster have greater than the
	 *            other.
	 * 
	 * @param r
	 * @return
	...
	public double[][] clusterCrossover1(double[][] father, double[][] mother, int num_vertex,
			ArrayList<Cluster> maxClusters, int numberOfCluster1, int numberOfCluster2, int[] minClusterVertices,
			int[] maxClusterVertices, Random r, Windows windows1) {
		double[][] Tree = new double[num_vertex][num_vertex];
		double[][] clusterWeightMatrix1, clusterWeightMatrix2;
		double[][] spanningTreeOfCluster;
		int numberClusterVertex = 0;
		...
		for (int i = 0; i < numberOfCluster; i++) {
			...
			clusterWeightMatrix1 = initilizeChromosome.buildClusterWeightMatrix(father,
					maxClusters.get(i).getCluster());
			clusterWeightMatrix2 = initilizeChromosome.buildClusterWeightMatrix(mother,
					maxClusters.get(i).getCluster());

			spanningTreeOfCluster = primRSTcrossover(clusterWeightMatrix1, clusterWeightMatrix2, minClusterVertices[i],
					maxClusterVertices[i], r);

			// debug
			// windows1.runWindow(" cluster thu " + i);
			// Paint p = new Paint();
			// p.setPaint(spanningTreeOfCluster, ReadFiles.vertices,
			// ReadFiles.clusters, minClusterVertices[i],0, 0, ReadFiles.root);
			// windows1.addPaint(p);

			// convert to the graph tree
			for (int j = 0; j < numberClusterVertex; j++) {
				for (int k = 0; k < numberClusterVertex; k++) {
					Tree[maxClusters.get(i).getCluster().get(j)][maxClusters.get(i).getCluster()
							.get(k)] = spanningTreeOfCluster[j][k];
				}
			}
		...
		spanningTreeOfCluster = primRSTcrossover(clusterWeightMatrix1, clusterWeightMatrix1, numberOfCluster1,
				numberOfCluster2, r);
		// convert to spanning tree of Graph
		int[] indexCluster = new int[numberOfCluster];
		for (int i = 0; i < numberOfCluster; i++) {
	 ...
	 * @param mother
	 * @param minNum_vertex
	 *            : number of vertices in cluster have number of vertices less
	 *            than the other.
	 * @param maxNum_vertex
	 *            : number of vertices in cluster have number of vertices
	 *            greater than the other.
	 * @param r
	 * @return tree after apply primRSTcrossover
	 */
	private double[][] primRSTcrossover(double[][] father, double[][] mother, int minNum_vertex, int maxNum_vertex,
			Random r) {
		...
		// which is connected to the others vertices aren't in smaller tree to
		// queue
		for (int i = 0; i < minNum_vertex; i++) {
	...
	/**
	 * find cycle in graph using DFS algorithm, we traversal tree by Using the
	 * BFS, if we have the reserving edges in graph then we have the cycle in
	 * that graph, then stop Traversal Tree after find the cycle already yet, we
	 * use the previous vertex of the end vertex of that cycle to find the
	 * others vertex based on the previous vertex( recur), over and over again
	 * until the start vertex of that cycle, then return the path.
	 * 
	 * @param Graph
	 *            : that graph which have cycle inside
	 * @param num_vertices
	 *            : the number of vertices in that graph
	 * @param startVertex
	 *            : the start vertex to travel tree
		...
		while (!stack.isEmpty() && flag) {
			...
			while (i < num_vertices) {
				if (Graph[element][i] > 0 && visited[i] == 0) {
					...
					// traversal tree DFS until detect the reserving edges then
					// stop. return the path.

					for (int j = 0; j < num_vertices; j++) {
						if ((preVertex[j] != -1) && (Graph[preVertex[j]][element] > 0)
								&& (preVertex[j] != preVertex[element])) {
							int temp = element;
							path.add(element);
	...
	public ArrayList<int[]> pruferNumberCrossover(int[] father, int[] mother, int genLength, Random rnd) {
		...
		// System.out.println("do dai genegenth" + genLength);
