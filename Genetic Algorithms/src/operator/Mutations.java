...
public class Mutations {
	...
	public double[][] mutationClusterTree(double[][] parent, int num_vertex, ArrayList<Cluster> clusters,
		...
		for (int t = 0; t < numGene; t++) {
			if (MyRandom.r.nextDouble() < muRate) {
				...
				clusterSpanningTree = mutationForEachClusters1(clusterWeightMatrix, minClusterVertices[indexCluster],
						maxClusterVertices[indexCluster], r);
				...
				// Paint p = new Paint();
				// p.setPaint(presentationTree, ReadFiles.vertices1,
				// ReadFiles.clusters1, numberOfCluster,
				// 0, 0, ReadFiles.root1);
				// windows.addPaint(p);
				//

				presentationTree = mutationForEachClusters1(presentationTree, ReadFiles.numberOfCluster,
						ReadFiles.numberOfCluster1, r);
	...
	public double[][] mutationClusterTreeGA(double[][] parent, int num_vertex, ArrayList<Cluster> clusters, Random r,
			double muRate) {
		Crossover crossover = new Crossover();
		InitializeChromosome initializeChromosome = new InitializeChromosome();
		double[][] offspring = new double[num_vertex][num_vertex];
	...
	public int[] mutationPrufer(ArrayList<Cluster> clusters, Random rnd, int[] pruferNumberCode, int[] startpoint,
			double muRate) {
		int numberOfClusters = clusters.size();
		int[] temp = pruferNumberCode;
		// int indexCluster = rnd.nextInt(numberOfClusters);
		// int numberClusterVertices =
		// clusters.get(indexCluster).getCluster().size();
		int indexCluster = rnd.nextInt(numberOfClusters);
		int numberClusterVertices = clusters.get(indexCluster).getCluster().size();
		while (numberClusterVertices < 3) {
			indexCluster = rnd.nextInt(numberOfClusters);
			numberClusterVertices = clusters.get(indexCluster).getCluster().size();
		}
		double numVertices = (1.0 / muRate);
		for (int i = 0; i < numVertices; i++) {
			if (rnd.nextDouble() < muRate) {
				if (indexCluster == 0) {
					temp = swap(rnd.nextInt(numberClusterVertices - 2), rnd.nextInt(numberClusterVertices - 2),
							pruferNumberCode);
				...
				temp[startpoint[numberOfClusters - 1] + numberOfClusters - 2 + indexCluster] = rnd
						.nextInt(numberClusterVertices);
			}
		}
		return temp;
	}
	...
	public int[] mutationPrufer1(ArrayList<Cluster> clusters, Random rnd, int[] pruferNumberCode, int[] startpoint,
			double muRate) {
		int numberOfClusters = clusters.size();
		int[] temp = pruferNumberCode;
		int indexCluster = rnd.nextInt(numberOfClusters);
