    ...
    for( int i = 0; i < ReadFiles.numberOfCluster; i ++){
    	for( int j = 0; j < ReadFiles.numberOfCluster; j++){
    		if(clusterSpanningTree[i][j] > 0){
    			tree[clusterRepresentation.getCluster().get(i)][clusterRepresentation.getCluster().get(j)] = clusterSpanningTree[i][j]; 
    		}
    	}
    }
     return tree;
...
//primRST
 public double[][] primRST(double[][] weightMatrix, int num_vertices ){
    ...
    while(contain.size() <num_vertices){
 	   ...
 	   Edge tempEdge = new Edge(edgeListAdjacence.get(indexRandomEdge).startVertice, edgeListAdjacence.get(indexRandomEdge).endVertice);
 ...
 public ArrayList<Edge> findEdge(int num_vertices, int vertice, ArrayList<Integer> contain, double[][] weightMatrix){
 ...
 public void findCycle( int start_vertice, int end_vertice, int number_vertices, int[][] weight_matrix, boolean[] visited, int[] pre){
