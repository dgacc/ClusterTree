			...
			ArrayList<Individual> tempPop = new ArrayList<Individual>();
			  while(tempPop.size() < defaultPopLength){
					...
					if (pop.get(par1).getSkillFactor() == pop.get(par2).getSkillFactor() || r < crossOverRate)
					{
						...
						child.setGene(mef2.crossover.clusterCrossover1(pop.get(par1).getGene(), pop.get(par2).getGene(), InitializeChromosome.maxNumberVertices,
				        		InitializeChromosome.maxClusters, ReadFiles.clusters.size(), ReadFiles.clusters1.size(), InitializeChromosome.minClusterVertices,
				        		InitializeChromosome.maxClusterVertices, MyRandom.r, windows1));
						if (r < 0.5)
	                        {
						...
						pop.get(par1).setGene(mef2.mutation.mutationClusterTree(pop.get(par1).getGene(), InitializeChromosome.maxNumberVertices, InitializeChromosome.maxClusters,
								InitializeChromosome.minClusterVertices, InitializeChromosome.maxClusterVertices, MyRandom.r, muRate));
						tempPop.add(pop.get(par1));
						pop.get(par2).setGene(mef2.mutation.mutationClusterTree(pop.get(par2).getGene(), InitializeChromosome.maxNumberVertices, InitializeChromosome.maxClusters,
								InitializeChromosome.minClusterVertices, InitializeChromosome.maxClusterVertices, MyRandom.r, muRate));
...
//			  Paint  p = new Paint();
//			  ArrayList<double[][]> caytest = initChromo.decodingTwoTree(newPop.get(0).getGene(), ReadFiles.clusters, ReadFiles.clusters1, 
//		        		ReadFiles.num_vertex, ReadFiles.num_vertex1, MyRandom.r);
//			  
//			  p.setPaint(caytest.get(1), ReadFiles.vertices1, ReadFiles.clusters1, ReadFiles.num_vertex1, 
